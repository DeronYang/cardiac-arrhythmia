...
Kernel::Kernel(int l, svm_node * const * x_, const svm_parameter& param) :
		kernel_type(param.kernel_type), degree(param.degree), gamma(
				param.gamma), coef0(param.coef0) {
	switch (kernel_type) {
	case LINEAR:
...
double Kernel::k_function(const svm_node *x, const svm_node *y,
	...
	case RBF: {
		...
		return exp(-1.0 / (2 * param.gamma * param.gamma) * sum);
	}
	case SIGMOID:
...
private:
	...
	bool be_shrunk(int i, double Gmax1, double Gmax2, double Gmax3,
			double Gmax4);
	void do_shrinking();
};
...
static void solve_nu_svc(const svm_problem *prob, const svm_parameter *param,
	...
	Solver_NU s;
	s.Solve(l, SVC_Q(*prob, *param, y), zeros, y, alpha, 1.0, 1.0, param->eps,
			si, param->shrinking);
	double r = si->r;
...
static void svm_binary_svc_probability(const svm_problem *prob,
		const svm_parameter *param, double Cp, double Cn, double& probA,
		double& probB) {
	int i;
	int nr_fold = 5;
...
int my_svm_predict(const svm_model *model, const svm_node *x) {
	int nr_class = model->nr_class;
	double *dec_values;
	if (model->param.svm_type == ONE_CLASS
			|| model->param.svm_type == EPSILON_SVR
		...
		dec_values = Malloc(double, nr_class*(nr_class-1)/2);
	int pred_result = my_svm_predict_values(model, x, dec_values);
	free(dec_values);
	return pred_result;
}
...
svm_model *svm_load_model(const char *model_file_name) {
	...
	LOGD("svm_type: %s", svm_type_table[model->param.svm_type]);
	LOGD("svm_kernel_type: %s", kernel_type_table[model->param.kernel_type]);
	LOGD("gamma: %g", model->param.gamma);
	LOGD("nr_class: %d", model->nr_class);
	LOGD("total_sv: %d", model->l);
	LOGD("rho: %g", model->rho[0]);
	LOGD("label: %d %d", model->label[0], model->label[1]);
	LOGD("nr_sv: %d %d", model->nSV[0], model->nSV[1]);
	LOGD("SV\n");
	int z = 0;
	LOGD("%g %d:%g %d:%g", model->sv_coef[0][z], model->SV[z][0].index,
			model->SV[z][0].value, model->SV[z][1].index,
			model->SV[z][1].value);

//	svm_load_result("/mnt/sdcard/group.dat", 35, 38);
//	svm_load_test_data("/mnt/sdcard/TestData.dat", 35, 38, 12);
	return model;
}
...
//第一条begin =1;
struct svm_result *svm_load_result(const char* result_file_name, int begin,
		int end) {
	if (begin <= 0 || end <= 0 || begin > end) {
		return NULL;
	}
	FILE *fp = fopen(result_file_name, "rb");
	if (fp == NULL)
		return NULL;

	svm_result *result = Malloc(svm_result,1);
	result->begin = begin;
	result->end = end;
	int len = end - begin + 1;
	result->result = Malloc(int,len);
	int tmp;
	...
	for (int i = 0; i < len; i++) {
		fscanf(fp, "%d", &result->result[i]);
//		LOGD("i: %d  result: %d", begin+i, result->result[i]);
	}
...
struct svm_test_data *svm_load_test_data(const char* data_file_name, int begin,
		int end, int width) {
	if (begin <= 0 || end <= 0 || begin > end || width < 1) {
		return NULL;
	}
	...
	svm_test_data *test_data = Malloc(svm_test_data,1);
	test_data->begin = begin;
	test_data->end = end;
...
//		fseek(fp, pos, SEEK_SET);
	...
	test_data->data = Malloc(svm_node*,l);
	svm_node *x_space = NULL;
	if (l > 0)
		x_space = Malloc(svm_node,l * (width+1));

	int j = 0;
	for (int i = 0; i < l; i++) {
		readline(fp);
		test_data->data[i] = &x_space[j];
...
//	for (int i = 0; i < l; i++) {
//		int j = 0;
//		while (test_data->data[i][j].index != -1) {
//			LOGD("i: %d    %d:%g", begin + i, test_data->data[i][j].index,
//					test_data->data[i][j].value);
//			j++;
//		}
//	}
	return test_data;
